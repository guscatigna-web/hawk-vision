...
export function NewEmployeeModal({ isOpen, onClose, onSave, employeeToEdit }) {
  ...
  return (
    // AJUSTE MOBILE: padding reduzido no container externo (p-2 vs p-4) e alinhamento
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 md:p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[95vh] md:max-h-[90vh]">
        
        {/* HEADER (Responsivo: p-4 vs p-6) */}
        <div className="bg-slate-900 p-4 md:p-6 flex justify-between items-center text-white shrink-0">
            <h2 className="text-lg md:text-xl font-bold flex items-center gap-3">
                <div className="p-1.5 md:p-2 bg-blue-600 rounded-lg"><User size={20} /></div>
        ...
        {/* BODY (Responsivo: p-4 vs p-8) */}
        <div className="p-4 md:p-8 overflow-y-auto custom-scrollbar">
            <form id="employeeForm" onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div className="col-span-1 md:col-span-2">
                        ...
                        <div className="relative">
                            ...
                            <input name="name" required defaultValue={employeeToEdit?.name} placeholder="Ex: João Silva" className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-700 transition-all text-sm md:text-base" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input name="email" type="email" required defaultValue={employeeToEdit?.email} placeholder="joao@hawk.com" className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-700 text-sm md:text-base" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input name="phone" defaultValue={employeeToEdit?.phone} placeholder="(00) 00000-0000" className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-700 text-sm md:text-base" />
                        </div>
                    </div>
                    ...
                    <div className="col-span-1 md:col-span-2">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                            {['Garçom', 'Cozinha', 'Caixa', 'Gerente'].map(role => (
                                <label key={role} className={`cursor-pointer border rounded-xl p-2 md:p-3 flex flex-col items-center gap-1 md:gap-2 transition-all ${selectedRole === role ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-sm ring-1 ring-blue-500' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:bg-slate-50'}`}>
                                    <input type="radio" name="role" value={role} checked={selectedRole === role} onChange={() => setSelectedRole(role)} className="hidden" />
                                    <span className="text-xs md:text-sm font-bold">{role}</span>
                                </label>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* SEGURANÇA */}
                    <div className="col-span-1 md:col-span-2 bg-amber-50 p-3 md:p-4 rounded-xl border border-amber-100">
        ...
        {/* FOOTER (Responsivo: p-4 vs p-6) */}
        <div className="p-4 md:p-6 border-t border-slate-100 bg-slate-50 flex gap-3 shrink-0">
            <button type="button" onClick={onClose} className="flex-1 py-3 text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl font-bold transition-colors text-sm md:text-base">Cancelar</button>
            <button form="employeeForm" type="submit" disabled={isSaving} className="flex-1 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 font-bold flex justify-center items-center shadow-lg transition-transform active:scale-95 disabled:opacity-50 text-sm md:text-base">
